...
test('when in development has hmre', (t) => {
  ...
  const plugins = config.plugins.map(p => p.match(/react[-|\w]+/))
  const pluginNames = plugins.map((p) => { if (p) { return p[0] } else { return p } })
  t.true(includes(pluginNames, 'react-hot-loader'))
})
